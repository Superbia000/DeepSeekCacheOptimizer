// DeepSeek Cache Optimizer v2.0 - 完整修正版
// 新增功能：
        ...
        if (isTargetEndpoint && options?.body) {
            try {
                ...
                if (body.messages && Array.isArray(body.messages)) {
                    const isDeepseekReq = url.includes('deepseek');
                    const skip = settings.deepseek_only && !isDeepseekReq;
                    
                    debugLog(`拦截到生成请求，DeepSeek: ${isDeepseekReq}, 消息数: ${body.messages.length}`,
                        skip ? '非 DeepSeek 端点，跳过优化' : '开始优化');
